...
```
FSR + 팔 올림    → UP (1)   (ml_000)
FSR + 팔 내림    → UP (1)   (ml_014) ← FSR 잡고 있으면 방향 무관하게 UP
FSR만 (팔 고정)  → DOWN (0) (ml_013)
팔만 (FSR 없음)  → DOWN (0) (ml_012)
idle             → DOWN (0)
```

### ml_014 — FSR 누르며 팔 내림 (역방향 FU)
- **수집 방법**: 팔을 먼저 올리고(FSR 없이), 대기 후 FSR 누르며 팔 내림. 반복.
- **735행, 36.7초**
- **FSR 범위**: 0 ~ 22846
- **라벨링** (ml_014_labeled.csv):
  - `1` (UP): FSR 고점 구간 (FSR 누르며 팔 내리는 중/유지) = 271행
  - `0` (DOWN): 나머지 (팔만 올림, idle) = 464행
  - **이유**: FSR 잡고 있으면 팔 방향 무관하게 FU (치료 지속)
